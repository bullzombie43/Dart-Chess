...
TEST_F(EngineTestFixture, GenerateCaptureMoves_StartingPosition) {
    // Starting position has no captures available
    board.set_position_fen("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1");
    Move moves[256];
    
    int capture_count = engine.generate_capture_moves(board, moves);
    
    EXPECT_EQ(capture_count, 0);
}

TEST_F(EngineTestFixture, GenerateCaptureMoves_SimpleCapture) {
    // White pawn can capture black pawn
    board.set_position_fen("rnbqkbnr/ppp1pppp/8/3p4/4P3/8/PPPP1PPP/RNBQKBNR w KQkq d6 0 1");
    Move moves[256];
    
    int capture_count = engine.generate_capture_moves(board, moves);
    
    // Should have exactly 1 capture: exd5
    EXPECT_GE(capture_count, 1);
    
    // Verify at least one move captures the d5 pawn
    bool found_capture = false;
    for (int i = 0; i < capture_count; i++) {
        if (moves[i].captured_piece != Piece::NONE) {
            found_capture = true;
        }
    }
...
TEST_F(EngineTestFixture, GenerateCaptureMoves_MultipleCaptures) {
    // Position with multiple captures available
    board.set_position_fen("r1bqkbnr/pppp1ppp/2n5/4p3/4P3/5N2/PPPP1PPP/RNBQKB1R w KQkq - 0 1");
    Move moves[256];
    
    int capture_count = engine.generate_capture_moves(board, moves);
    
    // Knight on f3 can capture e5, pawn on e4 can capture e5
    EXPECT_GE(capture_count, 1);
    
    // All returned moves should be captures
    for (int i = 0; i < capture_count; i++) {
        EXPECT_NE(moves[i].captured_piece, Piece::NONE);
    }
}

TEST_F(EngineTestFixture, GenerateCaptureMoves_EnPassant) {
    // Test en passant capture
    board.set_position_fen("rnbqkbnr/ppp1p1pp/8/3pPp2/8/8/PPPP1PPP/RNBQKBNR w KQkq f6 0 1");
    Move moves[256];
    
    int capture_count = engine.generate_capture_moves(board, moves);
    
    // Should include en passant capture exf6
    EXPECT_GE(capture_count, 1);
}

TEST_F(EngineTestFixture, GenerateCaptureMoves_NoIllegalCaptures) {
    // Position where a capture would leave king in check (pinned piece)
    board.set_position_fen("rnbqkb1r/pppp1ppp/5n2/4p3/2B1P3/8/PPPP1PPP/RNBQK1NR w KQkq - 0 1");
    Move moves[256];
    
    int capture_count = engine.generate_capture_moves(board, moves);
    
    // Verify all captures are legal (don't leave own king in check)
    for (int i = 0; i < capture_count; i++) {
        board.make_move(moves[i]);
        Color opposite = (board.sideToMove == Color::WHITE ? Color::BLACK : Color::WHITE);
        EXPECT_FALSE(board.is_in_check(opposite)) 
            << "Capture move leaves king in check - move is illegal";
        board.undo_move();
    }
}

TEST_F(EngineTestFixture, GenerateCaptureMoves_QueenCaptures) {
    // Test multiple directions of queen captures
    board.set_position_fen("r1bqkb1r/pppp1ppp/2n2n2/4p3/2B1P3/3Q4/PPPP1PPP/RNB1K1NR w KQkq - 0 1");
    Move moves[256];
    ...
    // Verify captures only, no quiet moves
    for (int i = 0; i < capture_count; i++) {
        EXPECT_NE(moves[i].captured_piece, Piece::NONE);
    }
}

TEST_F(EngineTestFixture, GenerateCaptureMoves_Promotion) {
    // Pawn promotion with capture
    board.set_position_fen("rnbqkb1r/pPpppppp/8/8/8/8/P1PPPPPP/RNBQKBNR w KQkq - 0 1");
    Move moves[256];
    
    int capture_count = engine.generate_capture_moves(board, moves);
    
    // Should include bxa8=Q, bxa8=R, bxa8=B, bxa8=N (and possibly bxc8 variations)
    EXPECT_GE(capture_count, 4);
}

TEST_F(EngineTestFixture, GenerateCaptureMoves_BlackToMove) {
    // Test with black to move
    board.set_position_fen("rnbqkbnr/pppp1ppp/8/3p4/4P3/8/PPPP1PPP/RNBQKBNR b KQkq - 0 1");
    Move moves[256];
    
    int capture_count = engine.generate_capture_moves(board, moves);
    
    // Black pawn can capture e4
    EXPECT_GE(capture_count, 1);
...
TEST_F(EngineTestFixture, GenerateCaptureMoves_TacticalPosition) {
    // Complex tactical position (Legal's mate setup)
    board.set_position_fen("r1bqk2r/pppp1ppp/2n2n2/2b1p3/2B1P3/3P1N2/PPP2PPP/RNBQK2R w KQkq - 0 1");
    Move moves[256];
    
    int capture_count = engine.generate_capture_moves(board, moves);
    
    // Should have several captures available
    EXPECT_GE(capture_count, 1);
    
    // Double-check: verify board state is unchanged after function call
    std::string fen_before = "r1bqk2r/pppp1ppp/2n2n2/2b1p3/2B1P3/3P1N2/PPP2PPP/RNBQK2R w KQkq - 0 1";
    EXPECT_EQ(board.get_fen(), fen_before);
}
...
//NO LONGER QUALIFIES BECAUSE WE ADDED QUIESSENCE SEARCH TO NEGAMAX AB, TO CONFIRM REPLACE QUIESCE WITH EVALUATE
...
TEST(EngineTest, AlphaBetaIsFaster) {
    ...
    // Results should match, NO LONGER TRUE BC QUIESCE
