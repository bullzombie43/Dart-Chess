...
//Taken from Polyglot Opening book http://hgm.nubati.net/book_format.html
...
//key=piece^castle^enpassant^turn; where ^ is XOR
...
//If the opponent has performed a double pawn push and there is now a pawn next to it belonging to the player to move then 
//"enpassant" is the entry from RandomEnPassant whose offset is the file of the pushed pawn (counted from 0(=a) to 7(=h)). 
...
constexpr int CASTLE_START = 768;
constexpr int EP_START = 772;
constexpr int TURN_START = 780;
...
inline int piece_to_zobrist_index(Piece piece) {
    ...
    return lookup[static_cast<int>(piece)];
}